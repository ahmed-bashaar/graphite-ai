...
- `render()` returns an HTML string. Always pass LLM-produced text through `escapeHtml` (`Renderable.ts`). Rendering is still a placeholder: `MessagePart` handles `text`/`code` only (no Markdown yet), and `Diagram.render()` (which adds a `source` field) returns its escaped source, because Mermaid rendering is async and lives in the UI layer (`renderMermaid`).
