...
- Agent tests (`src/agent/conversation.test.ts`) seed an Ollama provider and stub `fetch` to act as its server; type the mocks with `vi.fn<FetchLike>` so `mock.calls` type-checks. A one-line `Response.json({ message, done: true })` is a valid (single-chunk) Ollama stream, so stubs need no streaming.
...
- `conversation.ts`: `sendMessage(chatSessionId, text)` stores the user message (renaming a `NEW_CHAT_TITLE` chat after it) and then calls `reply(chatSessionId)`. `reply` answers only an unanswered last user message. It loads the history and project diagrams and runs them through a `ChatSession` + `AiAgent`, with `diagramTools` and `checkDiagrams`. The agent reacts to the session's `message` event, following the UML. The reply is saved with `parseReply`: diagrams are upserted by name into the project and referenced from the message, and the final `steps` are stored on the message (omitted when empty). It remembers the provider on the chat (`providerId`). With no provider configured it throws `NoProviderError`. Provider errors propagate to the caller.
...
| ↳ `chats/:chatId` | `ChatPage` → `Chat` (keyed by chat id) | Messages ordered by the `[chatSessionId+on]` index. Replies show a "thinking" status (`role="status"`). Saved agent messages with `steps` start with a collapsed "Worked through N steps" `<details>` (`AgentWork`). Errors show an alert with Retry (`reply()`), and an unanswered last message also offers Retry. Diagram parts render as cards with a live Mermaid preview, linking to the diagram. The composer has a Model picker (the chat's provider; defaults to the first provider) or an "Add a provider" link when none exist. Enter sends, Shift+Enter adds a newline |
...
- **Streaming replies**: show the agent's answer and work as they're generated instead of after a "thinking" wait, with a way to stop a reply. `LlmModel.step()` already streams (`onText`/`onThinking`) and the agent emits `progress` snapshots; the UI doesn't show them yet. Mermaid blocks should only be drawn once the reply has passed the diagram check.
...
- **Agent loop follow-ups**: OpenAI-compatible servers whose models reject `tools` have no fallback yet (Ollama's does). The loop's `maxSteps` isn't configurable in Settings.
