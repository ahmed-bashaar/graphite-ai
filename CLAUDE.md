...
- **Dexie** (IndexedDB) — the single database instance lives in `src/db.ts` as `db`, typed with `EntityTable`. Tables mirror the UML: `projects`, `chatSessions` (`title`, `draft`, optional `providerId`), `messages` (agent messages may carry `steps`: the `AgentStep`s the agent took before answering), `diagrams` (`type`, `name`, `source`, matching `Diagram` in `src/lib`). MessageParts are composed by their Message and stored inline in `messages.parts` (a `type`-discriminated union of `text`, `code` and `diagram-reference`), not in their own table. The `stores()` strings list only the primary key and indexed fields. Adding non-indexed fields needs no version bump. To change indexes, add a new `db.version(n + 1).stores(...)`; don't edit the existing version.
...
- **@anthropic-ai/sdk**: `AnthropicProvider` uses the official SDK (not raw fetch), streaming with `client.beta.messages.stream` + `finalMessage()`, with `dangerouslyAllowBrowser: true` because the app has no backend and users bring their own key. Ollama and OpenAI-compatible providers use plain `fetch` through `src/lib/http.ts`: `request` (throws on non-2xx with the server's message, including Gemini's array-wrapped errors), `requestJson`, `joinUrl`, and the stream readers `readLines` (NDJSON) and `readServerSentEvents` (SSE up to `[DONE]`). Before touching Claude API code, load the `claude-api` skill; model IDs, beta headers and parameters change often.
...
- Concrete providers: `AnthropicProvider`, `OllamaProvider`, `OpenAiCompatibleProvider` (OpenAI, OpenRouter, Groq, LM Studio, ...). Each file also holds its private `LlmModel` subclass, which maps `ChatTurn`s to its API and streams:
  ...
  - Ollama: `/api/chat` with `stream: true` (NDJSON), `message.thinking` as reasoning, `{ role: 'tool', tool_name }` results. Ollama tool calls have no ids, so ids are generated. A model that "does not support tools" is retried without tools (remembered per model); its answers are still checked.
  - OpenAI-compatible: SSE, tool-call pieces assembled by index, `reasoning_content`/`reasoning` as reasoning. It accepts a plain JSON completion from servers that ignore `stream`. Unparseable tool arguments become `{}`.

  `toChatTurns(history)` maps stored Messages to text turns: sender `'user'` is the user, anyone else is the assistant. `DiagramReference` parts become fenced ```` ```mermaid ```` source, so the model sees (and can revise) earlier diagrams. Only the current reply's tool transcript is sent; earlier replies are replayed as text (no thinking blocks), which is safe for Anthropic's preserved-thinking check because blocks are only ever dropped from the front.
...
- `conversation.ts`: `sendMessage(chatSessionId, text)` stores the user message (renaming a `NEW_CHAT_TITLE` chat after it) and then calls `reply(chatSessionId)`. `reply` answers only an unanswered last user message, and does nothing if the chat already has a reply in progress. It loads the history and project diagrams and runs them through a `ChatSession` + `AiAgent`, with `diagramTools`, `checkDiagrams` and an abort signal. The agent reacts to the session's `message` event, following the UML. Progress events go to `replies.ts`. The reply is saved with `parseReply`: diagrams are upserted by name into the project and referenced from the message, and the final `steps` are stored on the message (omitted when empty). It remembers the provider on the chat (`providerId`). With no provider configured it throws `NoProviderError`. Provider errors propagate to the caller. A stopped reply resolves quietly and saves nothing.
...
- **Agent loop follow-ups**: OpenAI-compatible servers whose models reject `tools` have no fallback yet (Ollama's does). Stop discards the partial answer rather than keeping it. The loop's `maxSteps` isn't configurable in Settings.
