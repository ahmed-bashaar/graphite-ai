...
/** A sent attachment in the user's message: the image itself, or the file's name. */
export function SentAttachment({ part }: { part: Extract<ContextPart, { type: 'attachment' }> }) {
  if (attachmentKind(part.mediaType, part.name) === 'image') {
    return <img src={dataUrl(part)} alt={part.name} className="max-h-64 max-w-full rounded-lg object-contain" />
  }
  return (
